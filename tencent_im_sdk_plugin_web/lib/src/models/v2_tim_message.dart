...
class V2TIMMessage {
  ...
  static Future<V2TimGroupMemberInfo> _getMemberInfo(String userID) async {
    ...
    List<V2TimUserFullInfo?> data = userInfoRes.data ?? [];
    V2TimUserFullInfo? userInfoDetails = data.isNotEmpty ? data.first : null;
  ...
  static List<V2TimGroupMemberChangeInfo> _convertGroupMemberInfoChanged(
    ...
    final memberChangeInfo = memberList.map((e) {
      final element = jsToMap(e);
      return V2TimGroupMemberChangeInfo(
          userID: element['userID'], muteTime: element['muteTime']);
    }).toList();
